...
function AskTypeCycle() {
  ...
  var chooseCycle = document.querySelectorAll(".MyIdregulierclass");
  chooseCycle.forEach((z) => {
    z.addEventListener("click", function () {
      var idMyId = z.id;
      console.log(idMyId);
      window.localStorage.setItem("myidMyId", JSON.stringify(idMyId));
    });
  });
  const UserConnectmyidMyId = localStorage.getItem("myidMyId");

  if (UserConnectmyidMyId === UserConnectmyidMyId) {
    document.getElementById("CalendrierMIdenstruel").style.display = "none";
    document.getElementById("CalendrierMIdenstruel1").style.display = "block";
    window.location.href = "./cycleCalendar/indexCalendar.html";
  } else {
    document.getElementById("CalendrierMIdenstruel").style.display = "none";
    document.getElementById("CalendrierMIdenstruel1").style.display = "block";
    if (UserConnectmyidMyId === "MyIdIrregulier") {
      window.location.href = "./cycleCalendar/indexa.html";
    }
  }
}
